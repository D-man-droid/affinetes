...
# How often to refresh the expansion instance list from R2 (seconds)
EXPANSION_REFRESH_INTERVAL = 300
...
class BreakerService:
    ...
    def __init__(
        ...
        # Load expansion instances from R2 (may be empty on first run)
        self.expansion_instances: List[Dict[str, Any]] = []
        self._load_expansion_instances()

    def _load_expansion_instances(self) -> None:
        """Load expansion instances from R2, sorted by instance_id for stable ordering."""
        instances = []
        try:
            paginator = self.s3.get_paginator('list_objects_v2')
            for page in paginator.paginate(
                Bucket=self.r2_bucket, Prefix=f"{EXPANSION_R2_PREFIX}/"
            ):
                for obj in page.get('Contents', []):
                    key = obj['Key']
                    if not key.endswith('.json') or '/claims/' in key:
                        continue
                    try:
        ...
        self.expansion_instances = instances
        print(f"Loaded {len(self.expansion_instances)} expansion instances from R2")
...
"""
    ...
    def decode_task_id(self, task_id: int) -> Dict[str, Any]:
        ...
        Routing:
        - task_id % 10 == 1 → SWE-bench Pro instance
        - task_id % 10 != 1 → Expansion instance (falls back to Pro if none loaded)
        Both wrap with modulo when task_id exceeds the source's total count.
        """
        seed = task_id
        ...
        if task_id % 10 != 1 and self.expansion_instances:
            exp_idx = task_id % len(self.expansion_instances)
            return {
                "swe_instance_idx": exp_idx,
                "swe_instance": self.expansion_instances[exp_idx],
                "bug_types": bug_types,
                "seed": seed,
            }

        # Pro instance (task_id % 10 == 1, or no expansion instances available)
    ...
    async def run(
        ...
        last_expansion_refresh = time.time()

        log("=" * 50)
        log("Starting breaker service")
        log(f"Machine ID: {self.machine_id}")
        log(f"Scanning from task_id={scan_from}")
        log(f"Expansion instances loaded: {len(self.expansion_instances)}")
        log("=" * 50)

        while max_tasks is None or tasks_processed < max_tasks:
            # Periodically refresh expansion instances from R2
            if time.time() - last_expansion_refresh > EXPANSION_REFRESH_INTERVAL:
                prev_count = len(self.expansion_instances)
                self._load_expansion_instances()
                if len(self.expansion_instances) != prev_count:
                    log(f"Expansion instances updated: {prev_count} → {len(self.expansion_instances)}")
                last_expansion_refresh = time.time()
